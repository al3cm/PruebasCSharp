...
namespace SuperCalculator
{
    public class MathRegex
    {
        ...
        {
            Regex fullRegex = new Regex(@"^-{0,1}\d+((\s+)" + operators + @"(\s+)-{0,1}\d+)+$");
            return (fullRegex.IsMatch(expression, 0) || IsNumber(expression) || IsOperator(expression));
        }
       ...
       public static bool IsSubExpression(string expression)
       {
           ...
           if (numberRegex.IsMatch(expression) && operatorRegex.IsMatch(expression))
               return true;
           return false;
       }
